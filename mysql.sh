#!/bin/bash
  mysqlbase=/usr/local/mysqlsecond      #mysql 安装程序路径
  mysqldata=/data/mysqlsecond            #mysql 数据库目录
  mysqlcnf=/data/mysqlsecond/cnf            #mysql 数据库目录 
...
-DDEFAULT_COLLATION=utf8_general_ci
 ...
 cp ${mysqlbase}/support-files/my-default.cnf   ${mysqlcnf}/my.cnf
 # cp ${mysqlbase}/support-files/mysql.server  /etc/init.d/mysqld

  cd ${mysqlbase}/scripts/
 ./mysql_install_db --basedir=${mysqlbase} --defaults-file=${mysqlcnf}/my.cnf --datadir=${mysqldata} --user=mysql
